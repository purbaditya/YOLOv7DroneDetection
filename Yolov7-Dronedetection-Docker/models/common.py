...
### Custom - Combination of a convolution and activation layer (ReLU)
...
### Custom - Module to  reshape tensors to patches of tensors or vice-versa
...
### Custom - Channel Attention Network
class Cat(nn.Module):
    # Channel attention block
    def __init__(self, c1, c2, e=2):  # ch_in, ch_out, number, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = nn.AdaptiveAvgPool2d((1,1))
        self.cv2 = ConvAct(c1, c_, 1, 1, 0)
        self.cv3 = nn.Conv2d(c_, c2, 1, 1, 0)  # act=FReLU(c2)
        self.s = nn.Sigmoid()
...
### Custom - Pixel Attention Network
class Pxat(nn.Module):
    ...
    def __init__(self, c1, c2, e=2):  # ch_in, ch_out, number, groups, expansion
        ...
        self.cv2 = nn.Conv2d(c_, c2, 3, 1, 1)  # act=FReLU(c2)
        self.s = nn.Sigmoid()
...
### Custom - Patch Attention Network
class Pat(nn.Module):
    # Patch attention block
    def __init__(self, c1, c2, e=2, k=16):  # ch_in, ch_out, number, groups, expansion, factor (number of patches)
        super().__init__()
        c_ = int(c2*k*k*e)  # hidden channels
        self.sh1 = Reshape(k,reconstruct=False)
        self.cv1 = nn.AdaptiveAvgPool2d((1,1))
        self.cv2 = ConvAct(c1*k*k, c_, 1, 1, 0)
        self.cv3 = nn.Conv2d(c_, c2*k*k, 1, 1, 0)  # act=FReLU(c2)
        self.s = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        return self.sh2(sh1*torch.reshape(s,(s.size(0),s.size(1),1,1)))
##### end of basic #####
...
##### custom classifier #####
### For classifying patches as a second stage classifier (not tested - no dataset created)
